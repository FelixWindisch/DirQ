...
) -> EdgeListGraph
{
    ...
    = rayon::join(
        ...
        //||{get_all_i_j_cofaces_alt(  q,  i, j, flag_complex, &simplex_map)}, //&simplicial_family[..])},
        ||{get_inclusion_edges( q, i, j,   &flag_complex, &simplex_map)});
    //let i_j_cofaces = get_all_i_j_cofaces(  q,  i, j, flag_complex[0].len(), &simplex_map, &simplicial_family[..]);
    //let (q_inclusions, inclusion_edges) = get_inclusion_edges( q, i, j,   &flag_complex, &simplex_map);
